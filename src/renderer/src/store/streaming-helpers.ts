  ...
  schedule(): void {
    ...
    this.rafId = requestAnimationFrame(() => {
      ...
      const segCopy: StreamingSegment[] = this.getSegments().map(s => ({
        ...
        ...(s.events ? { events: s.events.map(e => ({ ...e })) } : {}),
      }))
      this.flush(segCopy)
    })
  }
...
export function trimTrailingEmpty(segments: StreamingSegment[]): void {
  if (segments.length > 1) {
    ...
    if (last && !last.reasoning && !last.content && !last.toolCalls.length && !last.expertEvents?.length && !last.events?.length) {
      segments.pop()
    }
  }
}
...
export function computePersistSegments(segments: StreamingSegment[]): StreamingSegment[] | undefined {
  const nonEmpty = segments.filter(s => s.reasoning || s.content || s.toolCalls.length || s.expertEvents?.length || s.events?.length)
