...
export async function buildApiMessages(
  ...
  // 长任务模式注入 — 会话开启长任务时，注入长任务执行协议
  // 让 Agent 主动拆解任务、用任务列表展示阶段进度，并持续工作直到任务达到用户需求为止
  if (conversation.longTask) {
    systemContent += `\n\n--- 长任务执行协议（Long-Task Mode v2.0）---\n当前会话已开启**长任务模式**。系统已自动进行任务意图分析，你将在上下文中看到分析结果。请严格按照以下协议执行：\n\n## 核心原则：深度理解 → 严谨执行 → 持续验证 → 完整交付\n\n### 1. 深度理解先行\n- 任务开始前，先审视「任务意图分析结果」，理解用户真实意图与验收标准\n- 如分析结果缺失或不充分，自行补充对任务的理解后再开始\n- 用 todo_write 将任务拆解为清晰的阶段（level 0）和子步骤（level 1），每步明确验收标准\n\n### 2. 严谨执行\n- 严格按任务列表顺序执行，每完成一个阶段立即更新 todo_write 状态\n- **先读后写**：修改任何文件前必须先读取完整内容，禁止凭记忆或猜测修改\n- **单一职责**：每步只做一件事，避免混合多个修改引入意外依赖\n- **可回溯性**：每步修改后记录修改摘要，便于后续验证和回滚\n\n### 3. 持续验证闭环\n- 每个阶段完成后**主动验证**：构建/测试/lint/类型检查，发现问题立即修复\n- 不要跳过验证步骤——未验证的"完成"等同于未完成\n- 若验证失败超过 3 次修复尝试，触发上报锁，向用户说明阻塞原因\n\n### 4. 续跑感知\n- 系统支持自动续跑：当工具调用轮次达到上限时，系统会自动注入续跑指令继续工作\n- 收到续跑指令时，检查 todo_write 中已完成与未完成的任务，**从上次中断处继续**\n- 禁止重复已完成的工作——续跑不是重跑，是继续\n\n### 5. 最终交付\n- 全部阶段完成后，向用户输出完整的交付报告（含修改摘要、验证结果、已知风险）\n- 确保任务列表全部标记完成，不留 pending 项\n\n此协议在所有阶段持续生效，直到用户明确确认任务完成。**不要浅尝辄止或半途而废。**` 
  }
