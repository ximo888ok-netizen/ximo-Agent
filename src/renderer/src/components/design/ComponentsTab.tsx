        ...
        {grouped.map(([category, comps]) => {
          ...
          return (
            <div key={category} className="mb-0.5">
              <button onClick={() => toggleCat(category)} className="flex w-full items-center gap-1 rounded-control px-1.5 py-1 text-left transition-colors hover:bg-bg-elevated-soft active:scale-[0.97]">
                {collapsed ? <ChevronRight size={11} className="text-text-muted" /> : <ChevronDown size={11} className="text-text-muted" />}
                <span className="text-caption font-semibold text-text-secondary truncate">{catCn}</span>
                <span className="ml-auto rounded-full bg-bg-elevated-soft px-1 text-caption text-text-muted">{comps.length}</span>
              </button>
              {!collapsed && (
...
function ComponentRow({ component, onClick }: { component: ComponentMeta; onClick: () => void }): React.ReactElement {
  ...
  return (
    <button aria-label={component.nameCn} onClick={onClick} title={component.nameCn} className="group flex items-center gap-1.5 rounded-control border border-border-subtle bg-bg-elevated-soft px-1.5 py-1 transition-[color,background-color,border-color,opacity,transform,box-shadow,filter] hover:border-accent/30 hover:bg-bg-elevated-soft active:scale-95">
      <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-control bg-accent/10">
        <Box size={11} className="text-accent" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="truncate text-caption font-medium text-text-primary group-hover:text-accent transition-colors">{component.nameCn}</div>
        <div className="truncate text-caption text-text-muted">{component.id}</div>
      </div>
      {hasDeps && (
        <div className="flex shrink-0 items-center gap-0.5">
          {component.dependencies.slice(0, 2).map(d => (
            <span key={d} className="rounded-control bg-accent/10 px-1 text-caption text-accent/70">{DEP_LABELS[d]?.[0] || d[0]}</span>
          ))}
        </div>
      )}
    </button>
...
function ComponentDetail({ component, onBack, onPreview, onUse, selectedComponentIds }: {
  ...
  return (
    <div className="flex flex-col p-2 overflow-y-auto">
      ...
      {/* 标题 */}
      <div className="flex items-center gap-2 rounded-card bg-gradient-to-br from-accent/15 to-purple-500/15 p-2.5 ring-1 ring-white/5">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-card bg-bg-elevated-soft">
          <Box size={16} className="text-accent" />
        </div>
        <div className="min-w-0">
          <h3 className="text-xs font-semibold text-text-primary">{component.nameCn}</h3>
          <p className="text-caption text-text-muted">{component.id} · {component.categoryCn}</p>
        </div>
      </div>
        ...
        <Section title="依赖库">
          <div className="flex flex-wrap gap-1">
            {component.dependencies.map(d => (
              <span key={d} className="rounded-control bg-accent/10 px-1.5 py-0.5 text-caption font-medium text-accent">
                {DEP_LABELS[d] || d}
              </span>
            ))}
          </div>
        ...
        <Section title={`Props（${component.props.length} 个）`}>
          <div className="flex flex-wrap gap-1">
            {component.props.map(p => (
              <code key={p} className="rounded-control bg-bg-elevated-soft px-1 py-0.5 text-caption text-text-secondary">{p}</code>
            ))}
          </div>
        </Section>
      )}
      ...
      <button
        onClick={onPreview}
        className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-card bg-accent/10 px-2 py-2 text-caption font-medium text-accent transition-[color,background-color,border-color,opacity,transform,box-shadow,filter] hover:bg-accent/20 active:scale-[0.98]"
      >
        <Eye size={13} />
        预览组件
      </button>
      ...
      <button
        onClick={onUse}
        className={`mt-1.5 flex w-full items-center justify-center gap-1 rounded-card px-2 py-2 text-caption font-medium transition-[color,background-color,border-color,opacity,transform,box-shadow,filter] active:scale-[0.98] ${selectedComponentIds.includes(component.id) ? 'bg-accent/20 text-accent ring-1 ring-accent/40' : 'bg-bg-elevated-soft text-text-secondary hover:bg-bg-elevated'}`}
      >
        {selectedComponentIds.includes(component.id) ? <><Check size={13} /> 已加入选择</> : <><Plus size={13} /> 加入选择</>}
      </button>
    </div>
  )
}
