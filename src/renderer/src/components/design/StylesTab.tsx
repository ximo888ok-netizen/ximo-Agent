          ...
          return (
            <div key={category} className="mb-0.5">
              <button onClick={() => toggleCat(category)} className="flex w-full items-center gap-1 rounded-control px-1.5 py-1 text-left transition-colors hover:bg-bg-elevated-soft active:scale-[0.97]">
                ...
                <span className="text-caption font-semibold text-text-secondary truncate">{category}</span>
                <span className="ml-auto rounded-full bg-bg-elevated-soft px-1 text-caption text-text-muted">{styles.length}</span>
              </button>
              {!collapsed && (
...
function StyleRow({ style, onClick }: { style: StyleEntry; onClick: () => void }): React.ReactElement {
  ...
  return (
    <button aria-label={style.name} onClick={onClick} title={style.name} className="group flex items-center gap-1.5 rounded-control border border-border-subtle bg-bg-elevated-soft px-1.5 py-1 transition-[color,background-color,border-color,opacity,transform,box-shadow,filter] hover:border-accent/30 hover:bg-bg-elevated-soft active:scale-95">
      ...
      <span className="min-w-0 flex-1 truncate text-caption font-medium text-text-primary group-hover:text-accent transition-colors">{style.name}</span>
...
function StyleDetail({ style, onBack, onUse }: { style: StyleEntry; onBack: () => void; onUse: () => void }): React.ReactElement {
  ...
  return (
    <div className="flex flex-col p-2 overflow-y-auto">
      ...
      <div className="rounded-card overflow-hidden ring-1 ring-white/5">
        <div className="flex items-center gap-2 p-2.5" style={{ backgroundColor: bg, color: fg }}>
          <div className="flex h-9 w-9 items-center justify-center rounded-card" style={{ backgroundColor: surface }}>
            <Palette size={16} style={{ color: accent }} />
          </div>
          <div>
            <h3 className="text-xs font-semibold">{style.name}</h3>
            <p className="text-caption opacity-60">{style.category}</p>
          </div>
        </div>
      </div>
      <Section title="风格 ID"><code className="rounded-control bg-bg-elevated-soft px-1.5 py-0.5 text-caption text-accent">{style.id}</code></Section>
      <Section title="核心色板">
        ...
        <div className="space-y-1">
          {['调用 design_style 获取 tokens.css', '粘贴 :root { ... } 到 <style>', '用 var(--accent) 等变量'].map((text, i) => (
            <div key={i} className="flex items-start gap-1.5">
              <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-accent/15 text-caption font-semibold text-accent">{i + 1}</span>
