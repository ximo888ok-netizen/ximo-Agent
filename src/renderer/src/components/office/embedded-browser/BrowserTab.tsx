...
export const BrowserTab = memo(function BrowserTab({
  tab, active, onNewTab, onTitleChange, onNavStateChange, registerWebview, isRecording, recordingEventsRef, onRecordedEvent,
}: BrowserTabProps): React.ReactElement {
  ...
  useEffect(() => {
    const wv = webviewRef.current
    if (wv) wv.style.display = active ? 'flex' : 'none'
  }, [active])
    ...
    const bindListeners = (): void => {
      ...
      const handleNavStateUpdate = (): void => {
        onNavStateChange(tab.id, {
          ...
          canGoBack: safeCall(() => wv.canGoBack()),
          canGoForward: safeCall(() => wv.canGoForward()),
        })
      ...
      const handleConsoleMessage = (e: Event): void => {
        ...
        if (!msg) return
        if (msg.startsWith('[XIMO_RRWEB]')) {
          try {
            const data = JSON.parse(msg.slice('[XIMO_RRWEB]'.length)) as Record<string, unknown>
            void window.api.skills.appendRrwebEvent(data)
          } catch { /* ignore */ }
          return
        }
        if (!isRecording || !msg.startsWith('[XIMO_REC]')) return
        try {
      ...
      const handleNewWindow = (e: Event): void => {
        const url = (e as CustomEvent<{ url: string }>).detail?.url
        if (url) { e.preventDefault(); onNewTab(url) }
      }

      const handleDidFailLoad = (e: Event): void => {
        const detail = (e as CustomEvent<{ errorCode: number; errorDescription: string }>).detail
      ...
      wv.addEventListener('did-start-loading', ((e: Event) => onNavStateChange(tab.id, { loading: true })) as EventListener)
      ...
      cleanupRef.current = () => {
        wv.removeEventListener('did-start-loading', ((e: Event) => onNavStateChange(tab.id, { loading: true })) as EventListener)
