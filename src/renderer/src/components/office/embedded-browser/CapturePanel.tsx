...
export function CapturePanel({ requests, isRecording, onClear, onClose }: CapturePanelProps): React.ReactElement {
  return (
    <div className="flex max-h-32 flex-col border-t border-border-subtle shrink-0">
      <div className="flex items-center justify-between px-2 py-1 bg-bg-surface-soft">
        <div className="flex items-center gap-1.5">
          <Radio size={11} className="text-accent" />
          <span className="text-caption font-medium text-text-secondary">API 抓包</span>
          <span className="text-caption text-text-muted">{requests.length}</span>
        </div>
        <div className="flex items-center gap-1">
          <button aria-label="清空" onClick={onClear} className="icon-btn rounded-control p-0.5" title="清空">
            <Trash2 size={11} />
          </button>
          <button aria-label="收起" onClick={onClose} className="icon-btn rounded-control p-0.5" title="收起">
            <X size={11} />
          </button>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {requests.length === 0 ? (
          <div className="px-3 py-2 text-center text-caption text-text-muted">
            {isRecording ? '等待 API 请求...' : '开启录制后自动捕获 XHR/Fetch 请求'}
          </div>
        ) : (
          requests.slice(-30).reverse().map((req) => (
            <div key={req.id} className="border-b border-border-subtle-soft px-2 py-1 text-caption">
              <div className="flex items-center gap-1.5">
                ...
                {req.duration !== undefined && (
                  <span className="shrink-0 text-caption text-text-muted">{req.duration}ms</span>
                )}
              </div>
            </div>
          ))
