  ...
  return (
    <div className="py-1.5">
      <div className="flex items-center justify-between gap-2">
        ...
        {/* 跟随主题色开关 */}
        <button aria-label={parts.linked ? '当前跟随主题色，点击切换为自定义' : '当前自定义颜色，点击切换为跟随主题色'}
          onClick={handleToggleLinked}
          className={`flex items-center gap-1 rounded-control px-2 py-1 text-caption font-medium transition-colors ${
            parts.linked
      ...
      {parts.linked ? (
        <>
          ...
          <div className="mt-1.5 flex items-center gap-2">
            <span className="w-10 text-caption text-text-muted">浓度</span>
            <input
              type="range"
              min={0}
              max={100}
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className="w-10 text-caption text-text-muted">混合</span>
            <div className="flex gap-1">
              {blendTargets.map((t) => (
                <button
                  key={t.value}
                  onClick={() => handleBlendTargetChange(t.value)}
                  className={`flex items-center gap-1 rounded-control px-1.5 py-0.5 text-caption transition-colors ${
        ...
        <div className="mt-1.5 flex items-center gap-2">
          ...
          <span className="text-caption font-mono text-text-secondary">{toHex(parts.customColor)}</span>
        </div>
        </>
      )}
    </div>
