  ...
  return (
    <div className="border-b border-border px-4 py-2">
      <button
        ...
        className="flex w-full items-center gap-2 text-xs transition-colors hover:bg-bg-hover-soft rounded-card px-1 py-0.5 active:scale-[0.97]"
      >
      ...
      {expanded && (
        <div className="mt-2">
          <div className="overflow-hidden rounded-panel border border-border-subtle shadow-glass">
  ...
  return (
    <div className="border-t border-border-subtle glass px-4 py-2">
      <div className="flex items-center justify-center gap-2">
        <button aria-label="撤销更改 (Git checkout)"
          onClick={handleUndo}
        ...
        <button aria-label="重做 (Git stash pop)"
          onClick={handleRedo}
          className="icon-btn flex h-8 w-8 items-center justify-center rounded-full border border-border"
        ...
        <button aria-label="提交更改 (Git commit)"
          ...
          className="chip flex h-8 items-center justify-center gap-1 rounded-full border-accent/30 bg-accent/10 px-3 text-accent hover:bg-accent/15 transition-[color,background-color,border-color,opacity,transform,box-shadow,filter] duration-fast hover:scale-105 active:scale-95"
          title="提交更改 (Git commit)"
        >
          <GitBranch size={13} />
          <span className="text-caption">提交</span>
        </button>
        <button
        ...
        <span className="ml-2 text-caption text-text-muted">AI 可能会出错，请核实</span>
      </div>
      {showMore && (
