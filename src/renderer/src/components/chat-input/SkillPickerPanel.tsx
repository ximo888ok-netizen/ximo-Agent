  ...
  return (
    <div className="relative" ref={skillPickerRef}>
      ...
      >
        <Play size={11} />
        技能
        {skills.length > 0 && <span className="text-caption opacity-60">({skills.length})</span>}
      </button>

      {/* 技能列表弹出面板 */}
      {showSkillPicker && (
        <div className="absolute bottom-full left-0 mb-2 w-[320px] max-h-[320px] rounded-panel border border-border-subtle bg-bg-elevated shadow-glass animate-fade-scale flex flex-col overflow-hidden z-50">
          ...
          <div className="flex-1 overflow-y-auto">
            {skills.length === 0 ? (
              <div className="px-3 py-4 text-center text-xs text-text-muted">
                暂无已录制技能
                <br />
                <span className="text-caption">打开浏览器后点击"录制"按钮开始录制</span>
              </div>
            ) : (
              skills.map((skill) => (
                <button
                ...
                >
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-control bg-accent/10 text-accent">
                    <Play size={11} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-medium text-text-primary">{skill.name}</p>
                    <p className="truncate text-caption text-text-muted">{skill.description || `${skill.steps.length} 步操作`}</p>
                  </div>
                  <span className="shrink-0 text-caption text-text-muted">{skill.invokeCount}次</span>
                </button>
              ))
            )}
          </div>
