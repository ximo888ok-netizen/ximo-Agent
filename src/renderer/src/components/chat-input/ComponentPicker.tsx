                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="ml-1.5 text-caption text-text-muted">{c.id}</span>
                      </div>
                ...
                return (
                  <div key={cat} className="mb-0.5">
                    ...
                    >
                      ...
                      <span className="text-caption font-medium text-text-secondary">{catCn}</span>
                      <span className="text-caption text-text-muted">{comps.length}</span>
                    </button>
                    {!collapsed && (
                      <div className="space-y-0.5 px-1 pb-1">
                        {comps.map((c) => {
                          ...
                          return (
                            ...
                            >
                              <div className="flex h-4 w-4 shrink-0 items-center justify-center rounded-control bg-accent/10">
                                <Box size={11} className="text-accent" />
                              </div>
                              <span className={`text-caption font-medium truncate ${isSelected ? 'text-accent' : 'text-text-primary'}`}>{c.nameCn}</span>
                              <span className="text-caption text-text-muted truncate">{c.id}</span>
                              {isSelected && <Check size={13} className="flex-shrink-0 text-accent" />}
                            </button>
                          )
                        })}
          ...
          {selectedComponentIds.length > 0 && (
            <div className="border-t border-border-subtle px-3 py-2">
              <div className="flex items-center gap-1 flex-wrap">
                <span className="text-caption text-text-muted">已选({selectedComponentIds.length})：</span>
